...
def profile():
    ...
    if request.method == 'POST':
        ...
        if not current_pass or current_pass != user['password']:
            flash('Current password is required and must be correct to update account.')
            return redirect(url_for('dashboard'))
...
def admin_edit(target_email):
    ...
    if admin_pass:
        if admin_pass != admin['password']:
            flash('Admin password is required and must be correct to perform this action.')
            return redirect(url_for('dashboard'))
    ...
    if new_email and new_email != target_email:
        ...
        if target_email == current_user:
            current_user = new_email
        flash('Admin email change applied.')
...
def admin_delete(target_email):
    ...
    if not admin_pass or admin_pass != admin['password']:
        flash('Admin password is required and must be correct to delete an account.')
        return redirect(url_for('dashboard'))
